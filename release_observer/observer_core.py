    ...
    for epoch in range(epochs):
        ...
        if epoch % log_every == 0:
            row = {
                ...
                "attribute_loss": float(attribute_loss.item()),
                "typed_structure_loss": float(structure_loss.item()),
            }
            ...
            print(
                ...
                f"attribute_loss={row['attribute_loss']:.6f} | "
                f"typed_structure_loss={row['typed_structure_loss']:.6f}"
            )

    return training_history


def score_test_edges(
    model,
    test_data,
    test_id_maps,
    test_edges,
    ...
    with torch.no_grad():
        ...
        for edge in test_edges:
            ...
            novelty = get_relation_novelty(edge_type, train_relation_counts)
...
def compute_metrics(edge_rows):
    labels = [row["label"] for row in edge_rows]
    typed_action_scores = [row["typed_action_risk"] for row in edge_rows]
    edge_error_scores = [row["edge_error"] for row in edge_rows]

    metrics = {
        "sklearn_available": SKLEARN_AVAILABLE,
        "num_edges_scored": len(edge_rows),
        "num_normal_edges": int(sum(1 for label in labels if label == 0)),
        "num_suspicious_edges": int(sum(1 for label in labels if label == 1)),
    }

    if SKLEARN_AVAILABLE and len(set(labels)) == 2:
        metrics["typed_action_risk_roc_auc"] = float(
            roc_auc_score(labels, typed_action_scores)
        )
        metrics["typed_action_risk_pr_auc"] = float(
            average_precision_score(labels, typed_action_scores)
        )
        metrics["edge_error_only_roc_auc"] = float(
            roc_auc_score(labels, edge_error_scores)
        )
        metrics["edge_error_only_pr_auc"] = float(
            average_precision_score(labels, edge_error_scores)
        )
    else:
        metrics["typed_action_risk_roc_auc"] = None
        metrics["typed_action_risk_pr_auc"] = None
        metrics["edge_error_only_roc_auc"] = None
        metrics["edge_error_only_pr_auc"] = None

    return metrics


def aggregate_node_risk(edge_rows):
    ...
    for row in edge_rows:
        ...
        for node_key in [src_key, dst_key]:
            node_action_risk[node_key] = max(
                node_action_risk[node_key],
    ...
    ranked_nodes = [
        {
            "node": node_key,
            "max_typed_action_risk": risk_score,
        }
        for node_key, risk_score in sorted(
    ...
    return ranked_nodes, node_supporting_edges
...
def write_release_report(path, metrics, edge_rows, ranked_nodes):
    top_edges = edge_rows[:8]
    top_nodes = ranked_nodes[:8]

    with open(path, "w") as file:
        file.write("# Aegis-HGX Release 1: The Observer\n\n")

        file.write("## Summary\n\n")
        file.write(
            "Release 1 is a static heterogeneous graph anomaly detector. "
            "It trains on normal typed cybersecurity behavior and scores a separate "
            "test snapshot containing normal actions plus injected suspicious actions.\n\n"
        )
        ...
        file.write("\n## Top Risky Typed Actions\n\n")
        for idx, row in enumerate(top_edges, start=1):
            label = "SUSPICIOUS" if row["label"] == 1 else "NORMAL"
            file.write(
                f"{idx}. **{row['src']} {row['relation']} {row['dst']}** "
                f"({label}) — typed_action_risk={row['typed_action_risk']:.4f}, "
                f"edge_error={row['edge_error']:.4f}, "
                f"source_risk={row['source_risk']:.2f}, "
                f"destination_risk={row['destination_risk']:.2f}, "
        ...
        file.write("\n## Top Risky Nodes\n\n")
        for idx, row in enumerate(top_nodes, start=1):
            file.write(
                f"{idx}. **{row['node']}** — "
                f"max_typed_action_risk={row['max_typed_action_risk']:.4f}\n"
            )

        file.write("\n## Limitations\n\n")
        file.write("- Synthetic fallback data is used for this release.\n")
        file.write("- The detector is static and does not model time yet.\n")
        file.write("- Node features are hand-crafted and small.\n")
        file.write("- The relation decoder is simple but interpretable.\n")
        file.write("- Release 2 should add temporal windows and memory.\n")


def run_release(output_dir="release_observer/artifacts"):
    ...
    print("\n================ RELEASE 1: THE OBSERVER ================")
    print("Training on normal graph. Scoring separate test graph.")
    print("\nTrain metadata:")
    print(train_data.metadata())
    print("\nTest metadata:")
    print(test_data.metadata())

    training_history = train_observer(
        model=model,
        train_data=train_data,
        schema_edge_types=SCHEMA_EDGE_TYPES,
    )

    train_relation_counts = count_positive_edges_by_relation(
        train_data,
        SCHEMA_EDGE_TYPES,
    )

    edge_rows = score_test_edges(
        model=model,
        test_data=test_data,
        test_id_maps=test_id_maps,
        test_edges=TEST_EDGES,
        train_relation_counts=train_relation_counts,
    )

    metrics = compute_metrics(edge_rows)
    ranked_nodes, node_supporting_edges = aggregate_node_risk(edge_rows)

    write_csv(output_path / "ranked_typed_actions.csv", edge_rows)
    write_csv(output_path / "ranked_nodes.csv", ranked_nodes)
    write_json(output_path / "observer_metrics.json", metrics)
    write_json(output_path / "training_history.json", training_history)
    write_release_report(
        output_path / "observer_report.md",
        metrics,
        edge_rows,
        ranked_nodes,
    )
    ...
    torch.save(
        {
            ...
            "metrics": metrics,
            "train_relation_counts": {
                str(key): value for key, value in train_relation_counts.items()
            },
        },
        output_path / "observer_model.pt",
    )

    print("\n================ TOP TYPED ACTIONS ================")
    for rank, row in enumerate(edge_rows[:10], start=1):
        label = "SUSPICIOUS" if row["label"] == 1 else "NORMAL"
        print(
            f"rank={rank:2d} | "
            f"label={label:10s} | "
            f"{row['src']:25s} -[{row['relation']:15s}]-> {row['dst']:25s} | "
            f"risk={row['typed_action_risk']:.4f}"
        )
